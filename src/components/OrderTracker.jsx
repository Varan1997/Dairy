            ...
            <div key={step.key} className={`flex items-center ${isLast ? "" : "flex-1"}`}>
              <div
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm transition ${
                  done ? "bg-brand-600 text-white shadow-soft" : "bg-cream-200 text-brand-900/30"
                }`}
              >
                {done ? "✓" : step.icon}
              </div>
              {!isLast && (
                <div className={`mx-1 h-0.5 flex-1 ${i < currentIndex ? "bg-brand-600" : "bg-cream-200"}`} />
              )}
            </div>
          );
        })}
          ...
          return (
            <div
              ...
              className={`text-center text-[11px] font-medium leading-tight ${isLast ? "w-9" : "flex-1"} ${
                done ? "text-brand-900" : "text-brand-900/40"
              }`}
            >
              {step.label}
            </div>
